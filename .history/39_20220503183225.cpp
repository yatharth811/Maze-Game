...
void Tile::render( SDL_Rect& camera )
{
    ...
    if( checkCollision( camera, mBox ) )
    {
		...
		if (mType >= 0 && mType <= 323)
			...
			secondImageTexture.render( mBox.x - camera.x, mBox.y - camera.y, &gTileClips2[ mType - 324] );
		
		else if (mType > 519 && mType <= 16903)
			thirdImageTexture.render( mBox.x - camera.x, mBox.y - camera.y, &gTileClips3[ mType - 520 ] );
		
		else if (mType > 16903 && mType <= 17002)
			fourthImageTexture.render(mBox.x - camera.x, mBox.y - camera.y, &gTileClips4[ mType - 16904 ] );
...
bool loadMedia( Tile* tiles[] )
{
	...
	if( !setTiles( tiles, "maps/layer2.txt") )
	{
